    ...
    <div class="table-responsive">
        ...
        <table class="customers_ts">
            <tr>
                ...
                <td>:</td>
                <td>
                    <select name="jenis_jual" id="jenis_jual" style="width : 100px;" onchange="set_penjual(this);set_penjual_change(this)">
                        <?php if($head->Jenis_Jual=="pemakai") {?>
            ...
            <tr>
                ...
                <td>
                    <select name="ongkir" id="ongkir" style="width: 199%;" onchange="set_ongkir(this)">
                        <?php if(strlen($head->nama_ongkir) > 1){ ?>
                        <option value="<?php echo $head->id_ongkir ?>"><?php echo $head->nama_ongkir ?></option>
                        <?php } ?>
            ...
            <tr>
                ...
                <td>:</td>
                <td>
                    <span id="alamat"><?php echo $head->Alamat ?></span>
                </td>

                <td>Jumlah Min Order</td>
                <td>:</td>
                <td>
                    <input value="<?php echo $head->jumlah_min_order ?>" type="text" name="jumlah_min_order" id="jumlah_min_order" class="transparant" readonly style="text-align: right;">
                </td>
            </tr>
            <tr>
                <td>Jenis Stok</td>
                ...
                <td>:</td>
                <td>
                    <input value="<?php echo number_format($head->harga_ongkir)?>" type="text" name="harga_ongkir" id="harga_ongkir" class="transparant" readonly style="text-align: right;">
                </td>
            </tr>
        </table>
        <br>
            ...
            <tbody id="row_body">
                ...
                foreach ($list->result() as $data) {
                    ...
                    <tr>
                        <td><?php echo $no ?></td>
                        <td>
                            <button type="button" class="btn btn-danger btn-xs" id="btn_hapus_<?php echo $no ?>" onclick="hapus_row('<?php echo $data->NomorBarangKeluar ?>','<?php echo $data->KodeBarang ?>','<?php echo $data->Jenis ?>','<?php echo $jumlah_row ?>','<?php echo $data->Qty ?>','<?php echo $no ?>')">Hapus</button>
                        </td>
                        <td>
...
<script>
    ...
    function hapus_row(nomor,kodebarang,jenis,jumlah_row,qty,urut)
    {
    ...
    function  edit_data_update_head(kode){
    ...
    function update_header(kode)
    {
        setTimeout(function(){ 
            $.post('<?php echo base_url('barang_keluar/c_barang_keluar/update_head_hapus_row')?>',$("#form_data_input").serialize(),function(data){
                // table.ajax.reload();
                console.log("update head ok")
            });
        }, 1700);
    }
    ...
    function get_barang(data,urut,jenis)
    {
        ...
        qty_lama    = $("#qty_lama_"+urut).val()
        

        kode_barang_set = (jenis=="kode") ? (kode_barang) : (nama_barang);
        $.post('<?php echo base_url('barang_keluar/c_barang_keluar/get_detail_barang') ?>',{kode_barang : kode_barang_set, jenis : jenis,jenis_stok : jenis_stok,jenis_jual : jenis_jual,qty_lama : qty_lama},function(data){
            if(jenis=="kode"){
