    ...
    <div class="table-responsive">
        ...
        <table class="customers_ts">
            <tr>
                ...
                <td>:</td>
                <td>
                    <input type="hidden" name="nomor" id="nomor" readonly value="<?php echo $head->NomorBarangMasuk ?>">
            ...
            <tr>
                ...
                <td style="width : 300px;">
                    <select style="width: 99%;" name="supplier" id="supplier" class="select2">
                        <option value="<?php echo $head->id_supplier ?>"><?php echo $head->Nama_supplier ?></option>
        ...
        <table class="customers" style="width: 800px;">
            ...
            <tbody id="row_body">
                <?php
                $no =0;
                $jumlah_data = $list->num_rows();
                foreach ($list->result() as $data) {
                    $barang = $ci->get_barang_edit($data->KodeBarang);
                    $no++; ?>
                    <tr>
                        <td><?php echo $no ?></td>
                        <td>
                            <input type="hidden" name="kode_barang_lama[]" id="kode_barang_lama" value="<?php echo $data->KodeBarang ?>" readonly>
                            <input type="hidden" name="qty_lama[]" id="qty_lama" value="<?php echo $data->Qty ?>" readonly>
                            <button type="button" class="btn btn-danger btn-xs" id="btn_hapus_<?php echo $no ?>" onclick="hapus_row('<?php echo $data->NomorBarangMasuk ?>','<?php echo $data->KodeBarang ?>','<?php echo $jumlah_data ?>','<?php echo $data->Qty ?>')">Hapus</button>
                        </td>
                        <td>
                        ...
                        <td>
                            <input value="<?php echo number_format($data->Harga)?>" type="text" name="harga_barang[]" id="harga_barang_<?php echo $no ?>" style="width: 100px; text-align : right;" onkeyup="num_only(this);set_sub_total('<?php echo $no ?>')">
                        </td>
                        <td style="text-align: right;">
                            <span id="sub_total_<?php echo $no ?>"><?php echo number_format($data->SUB_TOTAL)?></span>
                            <script>
...
<script>
    function hapus_row(nomor,kodebarang,jumlah_data,qty)
    {
        pesan = (jumlah_data=="1") ? " Semnua Transaksi '"+nomor+"' Akan DiHapus?" : " Yakin Barang Akan Dihapus ?";
        if(confirm(pesan)){
            $.post('<?php echo base_url('barang_masuk/c_barang_masuk/hapus_row')?>',{nomor : nomor, kodebarang : kodebarang,jumlah_data : jumlah_data,qty : qty}, function(data){
                if(jumlah_data=="1"){
                    table.ajax.reload();
    ...
    function add_row()
    {
        ...
        row = '<tr>'+
                    '<td>'+i+'</td>'+
                    '<td>'+
                        '<input type="hidden" name="kode_barang_lama[]" id="kode_barang_lama" readonly>'+
                        '<input type="hidden" name="qty_lama[]" id="qty_lama" readonly>'+
                    ...
                    '<td>'+
                        '<select class="select2" name="kode_barang[]" id="kode_barang_'+i+'" style="width: 99%;" onchange="get_barang(this,'+i+',\''+'kode'+'\')">'+
                            '<option value="">Pilih</option>'+
                            <?php foreach ($barang_all->result() as $brg) { ?>
                                '<option value="<?php echo $brg->Kodebarang ?>"><?php echo $brg->Kodebarang ?></option>'+
                            <?php } ?>
                        '</select>'+  
                    '</td>'+
    ...
    function simpan_data(){
        ...
        $.ajax({
            url     : "<?php echo base_url('barang_masuk/c_barang_masuk/update_data') ?>",
            type    : "POST",
